# RS-защелка
1) Рассмотим сначала cхему и таблицу истинности RS-защелки:
<p align="center">
...
- Подача на входы одновременно R=1 и S=1 не имеет особого смысла. пПосле снятия S=R=1 и возврата к S=R=0 итоговое состояние может стать
  неопределённым и зависит от задержек в элементах (кто ‘успеет’ первым).

2) Код для RS-защелки :
``` verilog
module rs_latch (
    input r,
    input s,
